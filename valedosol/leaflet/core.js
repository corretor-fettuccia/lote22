    ...
    if (dados.botoes && dados.botoes.length > 0) {
        ...
        dados.botoes.forEach(btn => {
            if (btn.texto && btn.link) {
                html += `<a href="${escapeHtml(btn.link)}" target="_blank" rel="noopener noreferrer" style="background: rgba(0, 45, 128, 0.27); border: 1px solid #ffffff; color: #ffffff; padding: 8px 15px; border-radius: 30px; text-decoration: none; font-size: 13px; display: inline-block; text-align: center;">🔗 ${escapeHtml(btn.texto)}</a>`;
            }
        });
        html += `</div></div>`;
    }
